...
- [ ] **Zen Green colors match the token table in §1 — partly failing, see deviation D-04 below.** The
      primary green header, pale-green selected/success surfaces and soft gray-green read-only fields are
      all present as specified, but plain text links ("Back to My Tickets", the ticket number and summary
      links in the My Tickets rows) still render in the browser default blue instead of a Zen Green token.
      This item is deliberately left unchecked rather than ticked off, because it is not fully met.
...
### Known deviations: where the implementation does not yet match this spec

These were found by re-reading the screenshots against this document rather than by a test failing, which
is exactly the gap a visual checklist exists to catch. They are recorded here instead of being quietly
ticked off, and are carried into Lab 3 as follow-up work.

| # | Spec says | Implementation does | Evidence |
|---|---|---|---|
| D-01 | §8 Ticket Detail header shows Ticket Number, **Date**, Requester, Category, Related System, Priority, Status, Summary, Description | The Ticket Date field is absent; the header shows **Last Updated** instead | `ticket-detail/desktop-loaded.png` |
| D-02 | §8 attachment rows show filename, size, **uploader**, uploaded-at, badge | The uploader's name is not rendered on any attachment row | `ticket-detail/desktop-attachment-removed.png` |
| D-03 | §11 removed rows retain the removal reason **and removal time** | The removal reason is shown; the removal timestamp is not rendered (it is stored, per BR-22) | `ticket-detail/desktop-attachment-removed.png` |
| D-04 | §1 every colour comes from a Zen Green token | Plain text links render in browser-default blue rather than a token colour | every screenshot with a "Back to My Tickets" link |

None of these breaks an Acceptance Criterion or a business rule, and no automated test asserts them, which
is why the suite is green while the spec is not fully met. D-01 and D-04 are the two a marker is most
likely to notice, since both are visible in the Ticket Detail screenshots.
